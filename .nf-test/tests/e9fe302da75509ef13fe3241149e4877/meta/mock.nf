...
include { PREPROCESSING } from '/Users/w.huang/fspassemblypipeline/subworkflows/local/preprocessing/tests/../main.nf'
...
workflow {
    ...
    input = []
                ...
                input[0] = Channel.of([
                    [ id: 'SAMPLE_FSP_PAIRED_END', type: 'raw', single_end: false ],
                    [
                        file('/Users/w.huang/fspassemblypipeline/assets/dataset/B37-LG146-LP1_R1.fastq.gz', checkIfExists: true),
                        file('/Users/w.huang/fspassemblypipeline/assets/dataset/B37-LG146-LP1_R2.fastq.gz', checkIfExists: true)
                    ]
                ])
    ...
    //run workflow
    PREPROCESSING.run(input.toArray())
    
    if (PREPROCESSING.output){

        // consumes all named output channels and stores items in a json file
        PREPROCESSING.out.getNames().each { name ->
            serializeChannel(name, PREPROCESSING.out.getProperty(name), jsonOutput, params.nf_test_output)
        }	  
    
        // consumes all unnamed output channels and stores items in a json file
        def array = PREPROCESSING.out as List<Object>
        def i = 0
        array.each { output ->
