...
process COMPARISON {
    ...
    tuple val(meta), path(classifications)  // Receives from TIARA.out.classifications
    tuple val(meta), path(taxonomy_report)  // Receives from TIARA.out.classifications

    output:
    // TODO nf-core: Named file extensions MUST be emitted for ALL output channels
    tuple val(meta), path("${prefix}.{tsv,tsv.gz}")  , emit: taxononomic_classifications
    tuple val(meta), path("${prefix}.{tsv,tsv.gz}")  , emit: blob_tags
    ...
    tuple val("${task.process}"), val('comparison'), eval("comparison --version"), topic: versions, emit: versions_comparison
